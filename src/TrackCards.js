...
export default function TrackCards({ spotifyApi, track }) {
    ...
    async function getAudioFeatures() {
        const audioFeatures = await spotifyApi.getAudioFeaturesForTrack(track.trackId)
    ...
    async function createPlaylist() {
        if (!playlistCreated) {
            ...
            spotifyApi.createPlaylist('Vibefy Playlist', { 'description': `A playlist created by Vibefy based on ${track.trackName}`, 'public': 'false' })
            .then(res => {
                spotifyApi.addTracksToPlaylist(res.body.id, recommendations.tracks.map(track => track.uri))
    ...
    return (
        <Col>
            <Card id='track-card'>
                <Card.Img src={track.albumImage} style={{ height: '8em', width: '8em' }}/>
                <Card.Body>
                    <Card.Title id='track-name'>{track.trackName}</Card.Title>
                    <div id='artist-name' className='d-flex align-items-center text-muted'>
                        {track.trackExplicit && (<img id='explicit-symbol' alt='explicit symbol' src={explicitImage} />)}
                        {track.artistName}
                    </div>
                </Card.Body>
                
                <Button id='create-playlist' title='Create playlist' onClick={() => createPlaylist()}>&#43;</Button>
            </Card>
        </Col>
    )
}